...
struct StreightQuadPrismatic
{
    const int32_t ElementSize = 8;
    int32_t GlobalIdx[8];
    IntegratePoint3D points[8];

    void Print() const
    {
        for(int i = 0; i < 8; i++)
        {
            cout << "GlobalIdx = " << GlobalIdx[i] << "  ";
            cout << "i = " << i << " (x;y;z) = (" << points[i].x << ";" << points[i].y << ";" << points[i].z << ")\n";
        }
    }
};
...
private:
    ...
    int32_t ElementCount = 0; // Количество призматических элементов
...
public:
    ...
    */
    ...
    /*
        @param void
        @return int32_t - Количество элементов в сетке
    */
    inline int32_t GetElementCount() const noexcept { return ElementCount; }
